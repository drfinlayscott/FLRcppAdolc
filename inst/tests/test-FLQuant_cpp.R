...
test_that("FLQuant get and set data accessors",{
    flq_in <- FLQuant(rnorm(10*30*4*2*4*500), dim = c(10,30,4,2,4,500))
    flq_in2 <- FLQuant(rnorm(10*30*4*2*4*500), dim = c(10,30,4,2,4,500))
    # get
    expect_that(test_FLQuant_get_data(flq_in), is_identical_to(flq_in@.Data))
    # set
    flq_out <- test_FLQuant_set_data(flq_in, flq_in2@.Data)
    expect_that(flq_out@.Data, is_identical_to(flq_in2@.Data))
    expect_that(flq_out, is_identical_to(flq_in2))
    # Try with different dims - we're just changing the data
    flq_in3 <- FLQuant(rnorm(5*10*1*3*2*200), dim = c(5,10,1,3,2,200))
    flq_out2 <- test_FLQuant_set_data(flq_in, flq_in3@.Data)
    expect_that(flq_out2@.Data, is_identical_to(flq_in3@.Data))
    expect_that(flq_out2, is_identical_to(flq_in3))
})
